...
echo "-- Reset + tests"
if docker ps >/dev/null 2>&1; then make reset; else echo "SKIP: make reset (docker not reachable)"; fi
if docker ps >/dev/null 2>&1; then make test; else echo "SKIP: make test (docker not reachable)"; fi
