...
- **Feedback→behavior:** Recent learning_events (thumbs, outcomes) are summarized and injected into the chat/API task prompt as `recent_feedback_summary` (counts by kind, last negative/positive note). The model can adapt to what you approved or rejected.
